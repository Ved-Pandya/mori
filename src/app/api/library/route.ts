...
async function proxy(request: Request, path = "") {
  ...
  const response = await fetch(`${base}/api/library${path}`, { method: request.method, headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: request.method === "GET" ? undefined : await request.text(), cache: "no-store" });
