...
export default function Home() {
  ...
  if (view === 'reader' && selected && readerChapter) return <main className="reader">
    ...
    <section className="webtoon-pages" aria-label="Continuous vertical reader">{readerPages.map((page, index) => <img className="reader-image" id={`reader-page-${index}`} data-page={index} key={`${readerChapter.id}:${index}`} src={page} alt={`Page ${index + 1}`} loading={index < 3 ? 'eager' : 'lazy'} decoding="async"/>)}<div className="reader-end">End of chapter · Progress saved</div></section>
