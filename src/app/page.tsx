      ...
      <div className="content-inner"><div className="notice" role="status">{notice}</div>
        ...
        {view === 'browse' && <>
          ...
          <section className="source-browser">
            ...
            <div className="source-results">{sourceResults.map(manga => <button key={manga.hid} className="source-result" onClick={() => void openSourceTitle(manga)} aria-label={`View ${manga.title}`}>{manga.poster?.medium ? <img src={manga.poster.medium} alt="" loading="lazy"/> : <span>{manga.title[0]}</span>}<strong>{manga.title}</strong><small>{manga.latestChapter ? `Latest chapter ${manga.latestChapter}` : manga.type || 'View details'}</small></button>)}</div>
